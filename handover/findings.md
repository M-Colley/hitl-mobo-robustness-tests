...
| A2 | BLOCKER for Section 9 | The budget rule takes the sitting SD from GP noise in standardised units and combines it with a posterior in objective units. | budget_split.py:235 | Multiply by `outcome_transform.stdvs**2`, rerun without `--summary-only` and with the recorded k-grid and ρ | V | open |
