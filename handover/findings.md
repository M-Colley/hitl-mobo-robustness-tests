...
| E2, I3, I7 | MAJOR | Table 25 is typed by hand, has no price column, defines none of its fault models or remedies, and its "−979%" row divides by a reference cost near zero that changes sign. | main.tex:1462-1516 | Generate the table, add prices and definitions, report absolute differences for missing ratings | V, D | partly 2026-09-18: every fault and remedy in the table is now defined in one sentence each in the appendix preamble (spike, relay, cap, halo, lost rating, clipping, relevance pursuit); the table is still typed by hand, has no price column, and the -979% row is unchanged |
...
| C15 | BLOCKER | The draft uses the 2026 style and header, spills onto page 10, and carries four `\todo` markers. | main.tex:1-15, 33, 319, 675, 686 | Switch style, cut, resolve | V | fixed 2026-09-18: official 2027 kit; main text ends 93% down page 9 with four figures; all five author todos resolved (author block in git-ignored paper/authors.tex, three study citations, anonymous mirror URL, AI negative list, ethics statement from the original publications) and the todo macro removed. The mirror's own anonymity is C14/D3/I17. |
