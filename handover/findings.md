...
## 7. Completion round of 2026-09-23 (evening)

Everything section 6 left as "partly", "no script" or "not redone", plus the items deliberately left in the first pass.

| ID | what was done | status |
|---|---|---|
| C1/D7, I5/D8, I9/E6, E5, D9, B3, C6, I8 | the triage scripts behind these sentences are now in the repository, `scripts/review_checks/` (currency curvature and spline, manipulation intervals, structural extra-trial zeros, like-for-like Kendall's W, multi-objective front replay, nugget radius, ehmi sigma_f on two surfaces, companion ratio interval, augmentation contrast per dataset); `python scripts/run_review_checks.py` runs all nine and writes `output-boba/analysis/review/register_checks/`; `paper/COMMANDS.md` records it | fixed 2026-09-23 |
| D9 | the like-for-like scalar W is 0.1050 exactly; the paper said 0.11 by rounding half up and now says 0.105 | fixed 2026-09-23 |
| B3/D5 | the anchor caption now says the ehmi oracle is fitted to per-design means and the other two to individual ratings; on individual ratings ehmi's sigma_f is 0.315 and its noise 0.86 sigma_f (against 0.74) | fixed 2026-09-23 |
| B2 | the Student-t arm was rerun with the standard RBF kernel (`--likelihood student_t_rbf`, `run_boba_adapt_studentt_rbf.ps1`, 1,800 runs, `output-boba-adapt-studentt-rbf/`), so that only the likelihood and its variational fit differ from the standard GP. It recovers -58% [-123, -5] of the trajectory cost and -60% [-114, -10] of the deployed cost at a price of 3.4 and 5.5 points without error (Matern version: -2% and -13%, price 0.2 and 1.5), +5% [-8, 16] at 40% misclicks from trial 1, median 8 extra trials (21% never). The kernel change did not mask a benefit of the likelihood; both rows are in Table adapt and the appendix paragraph. The run started two minutes before the code stamp existed, so its `run_metadata.json` invocation is stamped by hand. The same round moved the BH correction of `analyse_boba_adaptations.py` to the merged file (one family over all 58 arms per response; a one-arm call used to give q = p); no star changes at 0.05 or 0.01 | fixed 2026-09-23 |
| MO oracles | multi-objective oracle selection redone on the fixed opticarvis and provoice data with the seven families (extra trees on both, CV R2 -0.16 and -0.12), merged into `output/best_oracle_models.json`; no result in the paper reads them | fixed 2026-09-23 |
| D1 | the BOBA parity and provenance tests no longer skip: `tests/fixtures/boba_reference.json` (BOBA commit 5bcc0349, 40 points per function, the four registry tables) and `tests/fixtures/bo_sim_backup_2026-09-14.json` (the 2026-09-14 simulator's choice lists and default multi-objective run, recovered from its bytecode) are used where the live source is absent, and checked against it where present; `tests/fixtures/build_reference_fixtures.py` regenerates them. Paths and the upstream URL are withheld in the fixtures | fixed 2026-09-23 |
| I18 | every sweep appends a code stamp (commit, whether `scripts/` or `datasets.json` were dirty, time, argv with the home directory masked) to `run_metadata.json` under `invocations`, so a resumed sweep keeps the stamp of each invocation. I19 (external timestamp of the preregistration) stays open | I18 fixed 2026-09-23 |
| tests | 32 new tests: `column_ranges` loading and parsing (19, incl. reversed bounds), `oracle_isolation` fractions (5), the adaptations merge (8, incl. arms without results). Two gaps they found were fixed: reversed bounds now raise a clear error, and `analyse_boba_adaptations.py` no longer crashes (and writes nothing) when no requested arm has results | fixed 2026-09-23 |
| audit of the post-fix text | twelve corrections: the deployed isolation ratio at 1 sigma is 0.25 [0.14, 0.78] (was rounded to 0.26 [0.15, 0.78]); across cells the isolation ratio is 0.13 to 0.35, so the abstract says "an eighth to a third"; deployed ratios are a seventh to a quarter; the companion reports a third to a half (0.33 to 0.46), not "about half"; from trial 21 the intervals overlap at every magnitude; the ehmi radius sensitivity is -11% (0.269 -> 0.241); one sentence mixed the two opticarvis sigma_f (the residual bound is 1.34 in the anchor's units, 0.81 in Table archival's); fidelity predicts the deployed ratio (Spearman -0.50) though not the trajectory one at 1 sigma; the detrended lag-one autocorrelation is negative in opticarvis; "never upward" became "never significantly upward"; the fitted rated-twice arm still costs the trajectory (-66% [-78, -49]); a remaining "usual" became "common" | fixed 2026-09-23 |
| style | the 28 main-text sentences over 45 words (27 by our count, one more by mechcheck's) were split without changing any number, citation or reference (checked mechanically); the main text still ends on page 9 | done 2026-09-23 |
| C14 (part) | the local user path is gone from every tracked `run_metadata.json` (68), the isolation configs and run configs, `stopping_metadata.json` and the known-function doc; the writers now record repository-relative paths (`bo_synthetic_error_simulation.py`, `oracle_isolation.py make-data`, the code stamp masks the home directory in argv). The new test fixtures withhold the BOBA URL and checkout path. Still open: LICENSE, `datasets*.json` data URLs, the older test files and `.git`, which the mirror's masking terms must cover | partly 2026-09-23 |
| REF008 | not applied, deliberately: `\autoref` would label appendix subsections "Section A.2" rather than "Appendix A.2", which the paper's own wording gets right | won't fix |
