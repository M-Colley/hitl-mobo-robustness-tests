...
\title{What Noisy Feedback Costs Bayesian Optimization, \\
and the One Number That Predicts It}
...
choice and, as Section~\ref{sec:incumbent} shows, not a neutral one.
...
Section~\ref{sec:manipulation} tests this reading directly and qualifies it.
...
literature is structurally obliged to do exactly that. Section~\ref{sec:incumbent}
...
Section~\ref{sec:incumbent}, is that a hypervolume objective has no single
...
(Section~\ref{sec:noisediag}), and the effect turns out to scale with the budget
(Section~\ref{sec:designchoices}) in a way a mechanism should explain. And the
...
\begin{itemize}
  ...
  \item The multi-objective arm (Section~\ref{sec:multiobjective}) is six
  ...
  \item The robust baselines are two (Section~\ref{sec:designchoices}), not a
  ...
  \item The onset ratio depends strongly on the budget --- $2.3\times$ at
    ...
    (Section~\ref{sec:designchoices}). Every onset number in this paper is a
    statement about a fifty-iteration study.
  \item The preregistered replication is void on a control failure
    (Section~\ref{sec:confirmatory}). The four claims clear their thresholds on
...
Section~\ref{sec:manipulation} builds landscapes that vary these properties
independently rather than trying to disentangle them by regression. Dimension
and ruggedness are null, and Section~\ref{sec:manipulation} shows the null on
