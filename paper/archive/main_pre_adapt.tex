...
magnitude and landscape interact. Telling the surrogate the true noise variance
removes only $5\%$ of the cost, $3\%$ or less at one SD and above, so the loss
is informational; changing the incumbent definition removes $16\%$ overall and
...
$1.6$--$2.6\times$ worse than the design the person experienced. On the same grid
the fitted-oracle design reports about a third of the cost, and improving its
oracles by up to $0.29$ held-out $R^2$ does not close the gap.
\end{abstract}
...
never do. Extra trials buy back small error, not large: at real raters' magnitudes, more
than a third of studies would not recover a clean study's result with four times
its budget. Error from trial 41 costs a 50-trial study no extra trials at the median, though $8$--$12\%$ of runs at $1\sigma$ and above never recover.
...
points) and an estimate of rater noise; in practice it would be computed from a
pilot or a fitted surrogate, which we have not tested. Its residuals are positive
...
acquisitions from the posterior-mean incumbent to the observed maximum removes $15.6\%$ of the excess-weighted cost ($14\%$ at $1\sigma$, two thirds or
more at $0.25\sigma$ and below) and $35$--$41\%$ for the
probability-of-improvement family: robustness rankings rank
...
rating scale makes small errors costlier and large ones marginally cheaper; qKG
and replication recover a third of the early-error cost at $1\sigma$ (Appendix~\ref{sec:designchoices}).
...
effort: at $1\sigma$ and above the four processes are within a third of each other. Choosing an acquisition function is worth about $2.8\times$ at $1\sigma$, but
the ranking transfers only as far as design spaces resemble each other, which at
a median $W$ of $0.247$ is not far. What does transfer is Equation~\ref{eq:frag}: from a pilot or a surrogate of the
design space and the rater noise, it predicts half the variation in damage on its own
($R^2 = 0.48$). When the error is in the design, recording what was applied removes up to half
of a small slip's cost, and the design a log recommends should be re-evaluated
before it ships.

The fitted-oracle design recovers the same qualitative structure at about a
third of the magnitude; a better oracle does not close the gap, and three
design spaces cannot say whether the gap is a property of human objectives or
of these studies (Section~\ref{sec:fitted}).

Two things remain unexplained: the onset effect, which is not a failure of noise
estimation (Appendix~\ref{sec:noisediag}) and grows with the budget
(Appendix~\ref{sec:designchoices}); and why the cost grows with $\optz$ faster
than \textsc{gain} allows (Section~\ref{sec:currency}) when $\optz$ is not causal
on its own (Appendix~\ref{sec:manipulation}).
...
cannot say which design's answer is closer to a person. The rater-noise anchor runs through a fitted oracle's $\sigma_f$; every onset
number describes a fifty-iteration study; extra-trial counts are censored at the
...
\paragraph{Methods built for the problem recover about a third.} All ten
model-based acquisitions above are ordinary ones run under noise, which says
which standard choice survives best but not how much a purpose-built method
would recover. Two baselines: the knowledge gradient, which values the
improvement in the posterior \emph{maximum} rather than in any observed value,
and the practical answer of spending half the budget re-asking about the
incumbent. At $1\sigma$ from the first rating they lose $9.2\%$ of the
achievable improvement against the standard acquisitions' $13.9\%$, and at
$5\sigma$, $16.2\%$ against $21.5\%$ (Table~\ref{tab:robust}). At the late onset
the two are close to indistinguishable. So a third of the early-error cost at
$1\sigma$, and a quarter at $5\sigma$, is recoverable by choosing a different
method, at a fixed evaluation budget --- which for replication means buying that
recovery with half as many distinct designs. The rest is not recoverable this
way. Equation~\ref{eq:frag} measures that residue: the information the error
destroys, rather than any optimizer's failure to cope with it.
