...
Most process changes do not help either (Appendix~\ref{app:adapt}). Rating the
first ten designs twice, qKG and replication do worse under error than the
standard process: in fifty trials the designs they give up cost more than the
averaging buys. The observed-max incumbent is the exception, recovering a fifth
of the cost at no price and cutting the extra trials one-SD error needs from
eight to six, though the design it deploys is worse.
...
($-8\%$ on the deployed design, interval $[-17, 6]$). Re-rating the incumbent
throughout the run and qKG fail against the standard process for the
same reason, as Appendix~\ref{sec:designchoices} describes.
...
deployed design's cost, $[0, 9]$, reaching $18\%$ $[5, 29]$ when large error
...
a $1\%$ slip from the first trial and $17\%$ of a $5\%$ slip, but at the late
onset, where the optimizer is working on a steep peak, it discounts exactly the
observations that locate the peak: $-129\%$ and $-205\%$ at $15\%$ and $40\%$.
...
$5\sigma$ the pilot underestimates $\frag$ by about a fifth of its value but ranks
