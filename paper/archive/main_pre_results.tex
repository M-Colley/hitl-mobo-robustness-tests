...
costs $2.8\%$. Magnitude, landscape and onset move the cost by $23\times$,
$8\times$ and $5\times$; the acquisition function and the kind of error, the two
factors practitioners usually attend to, move it by $2.8\times$ and $1.3\times$.
Measured rater noise in three human studies falls where the loss is $12$--$20\%$.
The expected loss from a single noisy greedy choice, computable before any
optimization is run, explains nearly as much of the variation as every landscape
descriptor and the error magnitude together, and conditional on it the magnitude
has no residual effect ($\beta = -0.07$, $p = 0.44$). Telling the surrogate the
true noise variance removes only $5\%$ of the cost, so the loss is informational;
changing the incumbent definition removes up to $41\%$, so robustness rankings
rank acquisition-and-incumbent pairs. On the same grid, the fitted-oracle design
reports about a third of the cost, and improving its oracles by up to $0.37$
...
published optimum the objective is exact and the regret is a real regret, and
the geometry of the landscape can be measured before the optimization is run and
...
\begin{enumerate}
  ...
  \item Measurements of that cost and of what drives it
    ...
    loss computable from the landscape alone in a fraction of a second.
  ...
  \item An input-error arm, in which the person applies the wrong design rather
    than giving the wrong rating (Section~\ref{sec:inputerror}), and six checks
    that each change one thing about the design, from the surrogate's noise
...
mean, which is the expected regret of a random design. It ranges from $1.25$ to
$56.8$, and that $45\times$ spread is what identifies which scale the cost of
...
posterior mean at visited points as their incumbent, the noise-robust choice
and, as Appendix~\ref{sec:incumbent} shows, not a neutral one.
...
\begin{itemize}
  \item \textbf{Model-free floor.} \texttt{Random} and \texttt{Sobol} never read
    ...
    all $12{,}800$ such comparisons it is exactly zero. This positive control
  ...
  \item \textbf{Uniform offset.} A constant added to every observation is
    ...
    incumbent. Driven with a shared noise stream this holds to
    $9 \times 10^{-12}$ over the initial design and the first three model-based
    candidates, after which floating-point differences are amplified by the
    acquisition optimizer; in the sweep, \textsc{bias} and \textsc{gaussian} at
    onset~0 agree to within $0.011$ at every magnitude.
\end{itemize}
...
Decomposing the variation by factor puts the practical conclusions in order.
Sweeping the magnitude over its two decades moves the cost by $23\times$;
changing the landscape at fixed magnitude moves it by $8.1\times$ (Powell
$0.038$ to Hartmann-6 $0.303$ at $1\sigma$); moving the onset, by $5.0\times$;
changing the acquisition function, by $2.8\times$; and changing the error
\emph{process}, by $1.32\times$. The two factors practitioners attend to most,
which acquisition to run and which kind of rater error to model, are the two
smallest. At matched magnitude and onset~0 the mean cost is $0.109$ for
\textsc{gaussian}, $0.113$ for \textsc{drift}, $0.110$ for \textsc{bias} and
$0.092$ for \textsc{ar}(1): systematic error is indistinguishable from random
error of the same size, and correlated error is if anything gentler, since a
correlated sequence is partly absorbed into the surrogate's mean. For predicting
damage, magnitude and timing are the only properties of the error that matter.

\paragraph{Is $1\sigma$ a lot?} We estimate within-rater noise in three
published human studies with a nearest-neighbour (semivariogram nugget)
...
between columns. The largest costs we measure occur at magnitudes real raters
produce. The caveat is that $\sigma_f$ can only be computed through a fitted
...
actually available. A half-SD error is one thing when the optimum stands $1.25$
...
from the first observation, with intervals that include one, so a given
$\sigma_e$ destroys a roughly constant \emph{fraction} of the achievable
improvement even though it is a far smaller share of that improvement on a tall
landscape; from iteration 21 it is $1.14$ to $1.15$, and a late error costs tall
landscapes somewhat more than proportionally. Appendix~\ref{sec:manipulation}
...
landscape-by-condition cells, nearly as much as the full set of pre-measured
landscape descriptors together with the error magnitude ($R^2 = 0.53$). Entered
...
$\beta = -0.07$ with $p = 0.44$. \textbf{Conditional on what one noisy choice
would cost on this landscape, how large the error is carries no further
information about what fifty sequential noisy choices will cost.} Dimension and
...
both of which a practitioner has before running a study. The residual terms say
where the one-shot approximation breaks down: $\optz$ and tail weight both retain
significant coefficients, which is where a surrogate fitted to fifty observations
averages noise down in a way a single choice cannot.
...
robustness and absolute performance (Table~\ref{tab:acq}). Every model-based arm
is far better than the model-free floor at $0.410$, so no arm wins by declining
to learn. The Friedman test over the twenty landscapes as blocks reaches
...
landscapes is $0.247$: the landscapes largely do not agree on an ordering. qUCB
...
would have produced a defensible, statistically significant and largely
non-transferable recommendation, and a study run on one design space can produce
nothing else. Appendix~\ref{sec:incumbent} shows that part of this ranking is
...
The fitted arm has no verified optimum, so its reference is the best value any
arm reached. That value is attainable by construction, which shrinks the
denominator and inflates the fitted arm's reported fraction, so this bias works
against the result below. With three design spaces against twenty, the fitted
intervals are also wide.

\textbf{The fitted design understates the cost of feedback error by roughly
threefold.} On this normaliser, at $1\sigma$ from the first rating, the
known-function arm puts the loss at $99\%$ of the achievable gain
(cluster-bootstrap interval $[55, 155]$) and the fitted arm at $33\%$
($[5, 75]$). At $0.05\sigma$ the fitted arm reports $1\%$ ($[-4, 3]$),
indistinguishable from no effect, where the exact functions report $10\%$
($[4, 18]$). The direction is the same at all four magnitudes and both onsets;
the intervals overlap at the larger magnitudes, so this is a consistent gap
rather than a separation in every cell.

\textbf{Oracle fidelity does not explain it.} Turning off the deployed jitter
augmentation improves the oracles by up to $0.37$ held-out $R^2$, most of what
they explain, since they top out near $R^2 = 0.55$. Rerunning the companion arm
without it changes the fraction destroyed by $+0.016$ (median $+0.006$) over the
$2{,}400$ cells the two arms share, and the augmentation-free arm is the worse of
the two in $52.6\%$ of them. If the gap were fidelity, it would close as the
oracle improved. What the fitted design changes is what the regret is measured
...
What gets recorded matters as much as where the person went. If the slip goes
unnoticed, the surrogate trains on $(x, g(x'))$ and the design eventually
deployed is the recorded $x$, never the $x'$ that earned its rating, so we score
the deployed design at the recorded point. A counterfactual arm records
...
against the floors' geometric cost.}
% Space held for the results table and two paragraphs, so that the page count
% measured while the arms run is the page count the finished paper will have.
\vspace{3.2in}
...
noise is an eighth of the injected value for the first twenty iterations, yet
...
\paragraph{Multi-objective BO is no more fragile.} On six \textsc{BoTorch}
...
Feedback error costs a real study something like a tenth to a fifth of the
improvement it could have had: worth reducing, but not the catastrophe the
framing sometimes implies. Classifying \emph{what kind} of error one's raters
make is largely wasted effort, since the four processes are within $32\%$ of each
other at matched magnitude. Choosing an acquisition function is worth about
$2.8\times$ in cost, and transfers only as far as a new design space resembles
the one it was measured on, which at $W = 0.247$ is not far. What does transfer
is Equation~\ref{eq:frag}: compute it for the design space and the measured rater
noise, and the expected damage follows.

The methodological finding may outlast the numbers. The fitted-oracle design
recovers the same qualitative structure at about a third of the magnitude, and a
better oracle does not close the gap (Section~\ref{sec:fitted}). A study that
measures robustness against a random-search estimate of its own optimum measures
a quantity whose ceiling it set itself, and that is cheap to avoid.
...
(Appendix~\ref{sec:designchoices}) in a way a mechanism should explain. And the cost of error grows with $\optz$ faster than the achievable
gain allows (Section~\ref{sec:currency}), although $\optz$ is not causal on
its own (Appendix~\ref{sec:manipulation}), which points at an interaction between signal
strength and how findable the optimum is that we have characterised but not
modelled.
...
buys, and Section~\ref{sec:fitted} shows that the choice of design changes the
answer, not which answer is closer to a person. The mapping of real rating noise
onto $\sigma_e$ runs through a fitted oracle's $\sigma_f$ and inherits its error,
so it bounds the range of interest rather than measuring it. Every onset number
describes a fifty-iteration study. The multi-objective arm is six problems at
...
acquisitions at five seeds and one slip distribution. The robust baselines are
two, not a survey. And the preregistered replication is void on a control
failure, so the claims survived an out-of-sample test committed to in advance
but cannot be called confirmed.
