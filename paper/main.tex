...
visited a better design than the one it ships, and none of the six
acquisition-side changes we ran reduces it. What reduces it is spending trials on
identification, or not spending them at all: shipping three candidate designs
instead of one recovers $24\%$ $[18, 30]$ of the cost with no extra trial, one
comparative sitting over the last sixth of the budget gains $0.02$ of the
achievable improvement, and four trials confirming the result cut
false improvement claims from $16\%$ to $0.5\%$, of which the confirming test rather
than the trials supplies more than half. The expected loss of one noisy choice
among random designs, computable from a ten-trial pilot, explains half of the
variation in excess regret across landscapes and magnitudes, against a sixth for
the error magnitude alone.
\end{abstract}
...
\begin{enumerate}
  ...
  \item \textbf{Where the cost sits.} Deployed regret splits into search loss
    ...
    costs, rising to $83\%$ for a large error arriving late. Under exact
    observation it is zero, and none of six acquisition-side changes reduced it.
    What works acts on the decision, not the search: shipping three designs
    instead of one recovers $24\%$ $[18, 30]$ for no extra trial, one final
    comparative sitting over a sixth of the budget gains $0.02$, and four
    confirmation trials cut false improvement claims from $15.7\%$ to $0.5\%$
  ...
  \item \textbf{A predictor from a pilot.} The expected loss of one noisy choice
    ...
    explains $R^2 = 0.52$ of the variation in excess regret across landscapes
    and magnitudes, against $0.16$ for the error magnitude alone, and stays
    informative beside the landscape descriptors (Section~\ref{sec:mediation}).
\end{enumerate}
...
part of the design, and twenty-three budget-neutral remedy arms.
...
simple and inference regret. What we add is not the distinction but its
measurement under rater error, and the fact that acquisition changes move one
...
are exact for $t \le t_0$. After that the optimizer sees $\tilde g_t$:
\begin{align}
...
$22.0\%$. Like for like, against the search loss at the same final trial, the
deployed design costs $1.7\times$ as much at $1\sigma$ and $2.5\times$ at $5\sigma$. The late onset, which looks nearly harmless on the
...
value is not. It is decided at a single moment by a single rating. Every
...
bootstraps. The two are on different time bases, so their difference is not the
selection loss of Section~\ref{sec:decomp}.}
\label{fig:dose}
\end{figure}
...
ratings must rank, and none of the six acquisition-side changes we ran
...
Table~\ref{tab:dose} gives its landscape-bootstrap interval.}
\label{fig:decomp}
\end{figure}
...
against $0.04$ for those indicators alone and $0.16$ once the error magnitude is
added, and nearly as much as the primary landscape descriptors and the magnitude
together ($0.53$). Out of sample, leaving one landscape out, it is ahead of them
($0.44$ against $0.41$). It stays informative beside them ($\beta = +0.31$
$[0.18, 0.45]$ in the interaction model) without making the magnitude redundant.
Computed on a GP fitted to the first ten trials of a clean run rather than on the
exact objective, it tracks the exact value at Spearman $0.94$ and predicts the
measured cost as well. The response is excess regret in landscape standard
deviations. As a fraction of the achievable improvement, which removes the scale
$\frag$ carries, the magnitude alone does better ($0.51$ against $0.46$), and
pooling magnitudes flatters both (Figure~\ref{fig:frag}). Its residuals
...
and averaged over conditions. ``Absolute loss'' is the noisy run's post-onset
trajectory regret as a percentage of the achievable improvement, a search
number, not the design that ships.}
\label{tab:acq}
...
\textbf{A better search does not ship a better design.} We ran six
...
expected improvement \citep{huang2006global} recovers $-5\%$ $[-21, +9]$ of the
cost of error against LogEI, a lower-confidence-bound incumbent $-27\%$, Thompson
sampling $-65\%$ $[-90, -41]$, a slip-aware acquisition $-4\%$ to $-35\%$, a
minimum-distance constraint $-13\%$; redistributing the same rating effort across
trials runs from $-7\%$ to $-21\%$. Nor does any clearly improve the trajectory:
augmented EI, the arm designed for noisy observations, recovers $+1\%$ $[-18, +18]$
there. Each is scored against LogEI. Against the mean of the ten acquisitions it
would read $+41\%$, a reference that carries the weakest arms' cost
(Appendix~\ref{app:adapt}).
...
\textbf{Identification is worth about a sixth of the budget.} Against the
standard process, in units of the achievable improvement, a sitting of $k=2$
gains $+0.010$ $[-0.003, +0.022]$, $k=3$ $+0.015$, $k=5$ $+0.020$, $k=8$
$+0.022$ $[+0.008, +0.034]$, $k=12$ $+0.021$, $k=16$ $+0.022$, and then falls
away (Figure~\ref{fig:kcurve}): $k=20$ $+0.009$, $k=25$ $-0.022$, $k=30$ $-0.054$.
The curve is the inverted U the two forces imply (a larger sitting is more
likely to contain the best visited design and leaves fewer trials to find one),
flat from $k=5$ to $k=16$ and peaking at about a sixth of $T$. The ceiling,
choosing the best $k$ per run in hindsight, is $+0.138$. This assumes only that
the error shared across one sitting cancels. If a comparison also halves the
idiosyncratic part the gains roughly double and the peak moves to $k=16$
(Appendix~\ref{app:endofstudy}).
Merely changing the ship rule, at no extra trial, is indistinguishable from the
standard process pooled over every magnitude ($+0.001$ $[-0.018, +0.018]$) but
removes $10.1\%$ $[4.6, 15.9]$ of \textsc{gaussian} error's cost at $0.25\sigma$
and above (Appendix~\ref{app:shiprules}). The extra trials buy more than the rule
does, but the rule costs nothing.
...
line is the hindsight-best $k$ per run. Pooled over the four error processes, all
four magnitudes and both onsets, with LogEI and qNEI. The looks carry the full
idiosyncratic SD.}
\label{fig:kcurve}
\end{figure}
...
at about a sixth of the budget.

\textbf{The cheapest remedy is to ship more than one design.} A study that
deploys the top three by the cautious rule, rather than the top one, recovers
$24\%$ $[18, 30]$ of the cost of error, and the top five $34\%$ $[28, 40]$,
for no extra human trial (Appendix~\ref{app:remedies}). That is more than any
procedure above buys with trials, and it follows directly from
Equation~\ref{eq:decomp}: the run has already visited a better design than the
one it ships, so widening the deliverable recovers part of the selection term
without touching the search. The number is what a later decision could recover
if it identified the best of the shortlist correctly, so it prices the
opportunity rather than delivering it. It pays only where such a decision is
cheaper than a rating, as a field trial or an objective measurement often is. Three further remedies that cost no rating trials are in
the same appendix: choosing on the ratings' ranks rather than their values
recovers $51\%$ $[36, 63]$ when $15\%$ of trials carry a gross fault and
$-1\%$ when none do; asking the rater how sure they were and using it as a
per-trial variance recovers $15\%$ $[6, 23]$; and the damage a saturating
scale does is predictable before the study from the headroom above its
ceiling.
...
here. A test asserts both the correction and the provenance of the original. Software and compute: BoTorch 0.18.2 on PyTorch 2.13
(a CUDA build, though the simulator uses the CPU only), Python 3.12 on Windows 11. A run
...
giving feedback on the research methodology and the experiments; implementing
methods, including the simulator, the analysis pipelines and the replay toolkit;
generating the synthetic data sets that those simulations produce; cleaning and
reformatting the archival rating datasets; helping to develop the conceptual
framework of search and selection loss; and interpreting results, with the
authors deciding every claim. We have not used generative AI tools for
translation, and the formulation and proof of mathematical claims and
...
drafting parts of the paper, editing it for readability, suggesting its
structure, creating and editing software code, creating the scientific figures
from the analysis outputs, brainstorming, and identifying and formatting a small
...
compute the deployed excess and must agree before a table is written; every
table is generated from the analysis outputs by one script; and the numbers
quoted in the prose were audited against those outputs in three independent
passes. The authors set the research questions, chose among the proposed
designs, and take responsibility for the final content of this work, including
...
removes $20.0\%$ of the excess-weighted cost, $24\%$ at $1\sigma$ and
$48$--$81\%$ at $0.25\sigma$ and below, and worsens the deployed design: robustness rankings rank
...
\paragraph{The fitted-oracle design reports a fraction of the cost.} On three
archival datasets it recovers $5$--$58\%$ of the cost the analytic arm measures, a
third at $1\sigma$ from the first rating, though that third is one dataset of three
($5\%$, $19\%$, $75\%$). The gap is neither the oracles' fidelity (up to
$0.29$ more held-out $R^2$ moves the cost by a median of $+0.006$) nor the
optimum estimate, which cancels in a difference of regrets
(Appendix~\ref{sec:fitted}).
...
magnitude, error from the first rating. Shape and shade both encode the magnitude.
Nine of the eighty cells cannot be placed on log axes, eight because $\frag$ is
exactly zero and one because the measured cost is negative. Both correlations in
the panel are over all eighty. The pooled value largely restates that both
quantities grow with $\sigma_e$, which is why the within-magnitude range is given
beside it.}
\label{fig:frag}
\end{figure}
...
landscape-by-condition cells, against $0.04$ for the onset and error-process
indicators alone and $0.16$ for those indicators plus the error magnitude,
and nearly as much as the primary
...
significant again beside it ($+0.17$ $[0.00, +0.31]$; forming the product from
uncentred logs instead reports this coefficient as a slope at $\optz = 1$, which
only powell is near, and flips its sign). \textbf{One noisy
...
Re-running with the observed maximum as the incumbent removes $20.0\%$ of the
excess-weighted cost overall ($24\%$ at $1\sigma$ from the first rating and
$20\%$ from trial 21, and about a half to four fifths at $0.25\sigma$ and below),
with six of eight conditions significant after FDR correction
...
$0.46\%$ and $65.7\%$. Two changes are bundled there, and they are worth
separating, because the standard process applies no test at all: it claims
whenever the best rating of the run exceeds the best of the initial design,
which is true unless the two coincide, so its power is $1.000$ and its size
$0.90$--$0.98$ in every cell. Claiming on the fresh mean alone, with the trials
spent but no test, already takes the false-claim rate from $15.7\%$ to $6.6\%$.
The $\alpha = 0.05$ test, which costs nothing, takes it from $6.6\%$ to $0.5\%$.
The trials buy the smaller half of the improvement. Power is also strongly
magnitude-dependent: $87.5\%$ at $0.05\sigma$ and $6.8\%$ at $5\sigma$ from the
first rating, so the pooled $50.7\%$ is carried by the cells where little is at
stake. All figures in this paragraph are the \textsc{gaussian} cells.
...
Clipping the response to the range actually observed recovers $6$--$35\%$ of the
...
Negative means the change does worse under error than the standard process. Each
arm is scored on the magnitudes and onsets it was run at, which differ between
arms, so a row is a share of its own reference's cost and the column is not a
ranking. The gross-fault rows are $0.25\sigma$ from the first rating only. The
lost-rating arm divides by a reference cost of $0.010$, near zero and negative on
nine of twenty landscapes, so it is given as an absolute loss in points of
achievable improvement rather than a share.
...
The three shared faults of this arm carry \emph{no} idiosyncratic noise: the
\textsc{drift} here is a bare ramp, the \textsc{offset} a bare constant and the
cap a bare truncation, whereas the sweep's processes of the same name add a
\textsc{gaussian} draw on top. A noise-free strictly monotone fault is precisely
what a comparison is immune to, so the $41$-against-none result above states
the construction rather than measuring a remedy; with the sweep's noise added
the comparison would inherit $\sqrt2$ of it. Read this arm as an upper bound.
A constant \textsc{offset} present from the first trial, with no noise added, moves
...
rater never changed), and scored on the held-out seeds. Two caveats bound what
the comparison can show. The standardised residual is not standard normal while
the rater behaves: its root mean square runs from $0.59$ to $11.4$ across
landscapes, so the tuned thresholds are large and the CUSUM and the GLR are both
dominated by the same largest squared residual, firing together on all but one
of the held-out runs. The budget is also common only in the pool: at a nominal
$5\%$ the held-out false-alarm rate is $0\%$ on branin and $29\%$ on shekel.
...
\end{equation*}
An arm that replaces the acquisition is scored against LogEI, the suite's
default. Scoring it against the mean of the ten standard acquisitions, as an
earlier draft did, is not like for like: the reference carries the four weakest
arms' cost, and augmented EI then "recovers" $+373\%$ at $0.05\sigma$ from trial
21, which would mean the arm under error beats the standard process with none.
The ten-mean figures are kept beside the LogEI ones in the analysis output.
Shares of a cost are likewise ratios of landscape means in units of the
achievable improvement; formed from raw landscape-SD values instead, the
landscapes with the largest $\optz$ dominate and the incumbent arm's overall
figure reads $15.6\%$ rather than $20.0\%$.
...
\section{Four remedies that cost no rating trials}
\label{app:remedies}

Every remedy in Appendix~\ref{app:budgetneutral} changes the loop. These four
change what the study deploys, how it reads the ratings it already has, or what
it asks the rater, and three of them are scored by re-reading runs that already
exist (\texttt{scripts/replay\_hitl\_remedies.py}), so they cost nothing to
evaluate as well as nothing to run. The estimand is the one of
Appendix~\ref{app:adapt}: a share of the standard process's cost of error on the
design the study deploys, as a ratio of landscape means with a
landscape bootstrap.

\begin{table}[htbp]
\centering
\caption{What a wider deliverable and a rank-based reading recover, on the
design the study deploys, as a share of the standard process's cost of error.
Shipping one design by the cautious rule is the baseline row, so every row above
it is what the wider deliverable buys. Twenty landscapes, LogEI and qNEI. The
main-sweep column pools four error processes, four magnitudes and both onsets.
Gross faults are $15\%$ of trials spiking at $20$ landscape SDs, at
$0.25\sigma$ from the first rating. The capped scale is cut at the landscape's
$0.9$ quantile, pooling $0.25\sigma$ and $1\sigma$ from the first rating.}
\label{tab:shortlist}
\small
\input{tables/shortlist}
\end{table}

\paragraph{A shortlist instead of a design.} Table~\ref{tab:shortlist} gives the
curve. Two things about it are worth stating plainly. The gain is entirely the
width of the deliverable: at $m=1$ the same rule recovers nothing on the main
sweep. And the score is the regret of the \emph{best} of the $m$, which a later
decision would have to find. It is therefore an upper bound on what shipping a
shortlist delivers, and the right way to read it is as the size of the prize for
any downstream discrimination that is cheaper than a human rating. Where no such
decision exists the number is unavailable, and a study should say so.

\paragraph{Choosing on ranks.} Refitting the surrogate on the ratings' normal
scores instead of their values makes the choice invariant to any strictly
monotone distortion of the scale and robust to a single wild rating. It costs
$1\%$ on the main sweep, where discarding the magnitudes throws information
away, and recovers $51\%$ $[36, 63]$ under gross faults and $36\%$ $[29, 43]$
against a saturating cap. At that fault rate it matches the relevance-pursuit
likelihood of Appendix~\ref{app:budgetneutral} while assuming nothing about the
fault. The contrast is the finding: a rank-based reading is not a general
improvement, it is an outlier remedy that happens to be free.

\paragraph{Asking the rater how sure they were.} A self-reported precision, used
as a per-trial observation variance, recovers $15\%$ $[6, 23]$ of the deployed
cost at a price of $-0.001$ (Table~\ref{tab:adapt}). It is worth setting beside
the known-noise arm, which is handed the \emph{true} noise variance and recovers
nothing: the difference is not accuracy but kind, since a constant is what the
GP already learns and a per-trial report says which trials were hard. We model
the report as the trial's squared error seen through a log-normal multiplier of
SD $0.5$, so an honest but coarse rater; a perfect report would price the
ceiling of the idea rather than a plausible version of it.

\paragraph{Screening the instrument.} A capped scale costs $185\%$ more than the
same noise on a scale that does not saturate, and how often the cap binds is
fixed by its quantile. What varies, and what a pilot can measure, is the
headroom: how far the reachable optimum sits above the ceiling. Across the
twenty landscapes that headroom predicts the extra cost at Spearman $0.65$, and
the extra cost doubles from $0.153$ in the low-headroom half to $0.316$ in the
high-headroom half. Re-anchoring the cap to the best design rated so far halves
the median damage, from $0.219$ to $0.104$. An instrument can therefore be
rejected before a participant is spent on it.

\paragraph{What did not work.} Six further ideas were built and measured, and
they are reported here because a remedy that fails is as useful as one that
works when the question is where the cost lives. Allocating the final sitting's
looks where the ranking is in doubt, by the standard best-arm rule
\citep{audibert2010best, kim2006selecting}, is never ahead of one look each
($+9.0\%$ against $+9.7\%$ at $k=8$): the gain is having looks, not where they
go. Sizing a study from the pilot's $\frag$ does no better than sizing it at
random, although any uneven allocation of a fixed budget beats an even one.
Interleaving fixed anchor designs to identify the rater's drift buys $+1\%$ and
costs trials. Rating the first five proposals late instead of early moves
nothing ($+1\%$), which rules out one explanation of the onset effect. Judging
each proposal beside the incumbent, so the error shared by the pair cancels,
\emph{loses} $27\%$: it pays $\sqrt2$ more fresh noise to remove faults the
loop already absorbs, since it standardises its targets and an argmax is
shift-invariant. An acquisition aimed at the ship rule rather than the posterior
maximum loses $107\%$, but its price without error is $+0.244$, the largest in
this paper, so it searches badly and its own idea is untested.
