...
\subsection{Metrics}
\label{sec:metrics}
We report two responses, and they are not interchangeable.

\textbf{Search loss} is the regret of the best design a run has \emph{visited},
$y^\star - \max_{t' \le t} f(x_{t'})$. Our primary response is its
\emph{post-onset per-iteration excess}: the noisy run's search loss minus that of
the identically seeded clean run, averaged over the iterations the error could
have affected. It is a time average, it describes the optimizer's progress, and
it is the response behind the dose-response curve, the currency test, the
descriptor model and the acquisition ranking.

\textbf{Deployed loss} is the regret of the design a study would actually ship:
the one with the best observed rating, scored at the final trial. It is search
loss plus the selection loss of Section~\ref{sec:decomp}, and at the final trial
it is about $2.6\times$ the final search loss. A study deploys a design, not a
trajectory, so every table that carries a search-loss number carries the deployed
number beside it, and we compare a final value only with a final value.

The two runs share their candidate-proposal random stream, so the
