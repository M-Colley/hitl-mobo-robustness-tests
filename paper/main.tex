...
magnitude and landscape interact. Decomposing the deployed regret locates that cost: $61.8\%$ $[53.6, 69.5]$ of it
is \emph{selection} loss --- the run visited a better design than the one it
ships --- the one term an acquisition function cannot reach, and identically zero
under exact observation, so a noiseless benchmark never sees it. Consistently,
...
identification --- one comparative sitting over the last third of the budget
gains $0.051$ $[0.028, 0.072]$ of the achievable improvement, and four trials
confirming the result cut false improvement claims from $15.7\%$ to $0.5\%$ ---
or modelling the fault explicitly, worth $22$--$38\%$ at near-zero price.
Telling the surrogate the true noise variance removes only $5\%$ of the cost,
...
\begin{enumerate}
  ...
  \item A decomposition of deployed regret into search and selection loss
    (Section~\ref{sec:decomp}): selection is $61.8\%$ of what error costs, is
    identically zero under exact observation, and is the term an acquisition
    cannot reach --- which is what reporting regret at the best \emph{observed}
    point hides.
...
Appendix~\ref{app:elicitation} measures where the line falls: at equal human cost
a comparison loop is \emph{exactly} unharmed by drift --- its shipped design
differs from its clean twin in no run --- and is no better under a saturating
scale and worse under inconsistency. We trade the realism of the objective for
...
ratings failed to identify among what it did find. An acquisition function acts
on the first term only: it chooses where to sample, and the second term is
already determined by the time the study picks a design to ship. The selection
...
\textbf{Most of what error costs is selection.} Pooled over the four error
processes, a deployed regret of $0.346$ of the achievable improvement splits into
$0.212$ of search loss and $0.135$ of selection loss, so selection is $38.9\%$
$[27.8, 52.6]$ of it. Against the identically seeded clean twin --- the part the
error caused rather than the part the problem is hard --- of $0.218$ of excess,
$0.083$ is search and $0.135$ selection: \textbf{selection is $61.8\%$
$[53.6, 69.5]$ of the cost of error}. The share is the same for every error
process ($60.7$--$63.6\%$) and rises with magnitude and lateness of onset, from
$29\%$ at $0.05\sigma$ from the first rating to $82\%$ at $5\sigma$ from trial 21.
...
rating, $-8.7\%$ $[-18, -1]$ for the posterior mean. The reason is structural:
the posterior ranking the visited designs was fitted on the same corrupted
ratings. Closing the term needs \emph{new} information, which is what
Section~\ref{sec:budget} spends trials on; Section~\ref{sec:acq} gives the other
half of the point, an acquisition that improves the search and not what ships.
...
\textbf{A better search does not ship a better design.} The sharper question is
whether an acquisition built for noisy observations helps at all. We ran seven
...
cost of error, a lower-confidence-bound incumbent $-27\%$ $[-42, -11]$, Thompson
sampling $-58\%$ $[-81, -40]$, a slip-aware acquisition $-4\%$ to $-35\%$, a
minimum-distance constraint $-13\%$ $[-23, -6]$; redistributing the same rating
effort across trials runs from $-7\%$ to $-21\%$. Augmented EI makes the point
alone: on the post-onset trajectory, the response an acquisition is designed to
improve, it recovers $+41\%$ $[+27, +53]$, the best of any arm we ran, and on the
design the study ships it recovers nothing. Under Equation~\ref{eq:decomp} the
term it improves is not the term that dominates.
...
Spending nothing extra and merely changing the ship rule gains $+0.001$
$[-0.018, +0.018]$, which is the selection result of Section~\ref{sec:decomp} in
another currency: the trials, not the rule, are what buys the recovery.
...
improvement (Appendix~\ref{app:budgetrule}) \emph{loses} to a fixed $k$: $+0.005$
$[-0.011, +0.022]$ against $+0.051$, because in $64\%$ of runs it declines the
sitting in favour of the posterior-mean design, the worst policy of all at
$-0.013$. Its utility is the surrogate's posterior, the surrogate was fitted on
corrupted ratings, and so it is confident about designs it should not be. We did
not find a rule that beats simply fixing $k$ at about a third of the budget.
...
design and the best initial design $k$ times each in counterbalanced order, and
the chosen design ships only if a one-sided paired $t$-test at $\alpha=0.05$
prefers it. This changes little about the design that ships (it recovers
$3.2\%$--$3.4\%$ of the cost) and a great deal about what the study may claim.
...
difference rather than the difference between policies. The rule reads only
ratings and the surrogate; the true objective enters only when the choice is
scored, against the realised regret of that $(\text{run}, k)$ in the replay.
...
The invariance holds exactly and buys exactly what it should. Under
\textsc{drift} the rating loop's shipped design differs from its clean twin in
$41$ of $60$ runs, at an excess of $0.040$ of the achievable improvement; the
comparison loop's differs in \emph{none}, at an excess of $0.000$ --- not
approximately, in no run. Under a saturating scale, a map that is monotone but
not strictly so, the comparison buys nothing ($-0.3\%$ $[-30, +27]$): two designs
above the cap are indistinguishable, and near the optimum that is exactly the
pair the study must tell apart. Under \textsc{gaussian} noise, drawn afresh per
judgement, the comparison is \emph{worse} by $26.5\%$ $[-79, +15]$, the direction
the $\sqrt2$ argument predicts. A constant \textsc{bias} present from the first
trial moves neither loop, in no run: a rating loop that standardises its outputs
