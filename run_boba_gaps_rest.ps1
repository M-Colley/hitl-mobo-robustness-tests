...
$log = "output-boba\gaps-rest.log"
if (-not (Test-Path "output-boba")) { New-Item -ItemType Directory "output-boba" | Out-Null }
"=== BOBA gap arms [budget/confirmatory/fitted] (re)started $(Get-Date -Format o) ===" | Tee-Object -FilePath $log -Append
...
# Shares the machine with run_boba_gaps_mo.ps1, which holds 12
# memory-hungry workers, so this one takes the rest rather than all of them.
$JOBS    = 10
